...
var Hooks = {
  ...
  LiveImgPreview: {
    mounted() {
      ...
      LiveUploader.getEntryDataURL(this.inputEl, this.ref, (url2) => {
        this.url = url2;
        this.el.src = url2;
      });
    },
...
var View = class {
  ...
  connectParams(liveReferer) {
    ...
    let manifest = dom_default.all(document, `[${this.binding(PHX_TRACK_STATIC)}]`).map((node) => node.src || node.href).filter((url2) => typeof url2 === "string");
  ...
  onLiveRedirect(redir) {
    ...
    let url2 = this.expandURL(to);
    this.liveSocket.historyRedirect(url2, kind, flash);
  }
  onLivePatch(redir) {
    let { to, kind } = redir;
...
var LiveSocket = class {
  constructor(url2, phxSocket, opts = {}) {
    this.unloaded = false;
    ...
    this.socket = new phxSocket(url2, opts);
